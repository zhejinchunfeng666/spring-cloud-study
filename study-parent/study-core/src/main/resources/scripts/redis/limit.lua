...
-- 保存上一次更新桶的时间的key
local LAST_TIME_KEY = key.."_time";
-- 获取当前桶中令牌数
local token_cnt = redis.call("get", key)
-- 桶完全恢复需要的最大时长
...
if token_cnt then   -- 令牌桶存在
    ...
    redis.call('set', key, token_cnt)
    redis.call('expire',key,reset_time)

    redis.call('set', LAST_TIME_KEY, last_time + multiple * add_interval)
    redis.call('expire',LAST_TIME_KEY,reset_time)
    return token_cnt
    ...
    --redis.call('set', LAST_TIME_KEY, now, 'EX', reset_time + 1);
     ...
     redis.call('expire',LAST_TIME_KEY,reset_time + 1)
    return token_cnt
end